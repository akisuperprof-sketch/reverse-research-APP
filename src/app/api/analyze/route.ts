...
export async function POST(req: Request) {
  try {
    ...
    if (!jsonMatch) {
      throw new Error("AIがJSON形式で回答しませんでした");
    }
    
    const analysisData = JSON.parse(jsonMatch[0]);

    // Supabaseに保存 (個別のテーブルに分割して保存)
    const { data: queryData, error: queryError } = await supabaseAdmin
      .from("search_queries")
      .insert([{ 
        keyword, 
        total_score: analysisData.scores.total,
        intent_stages: analysisData.intentStages,
        scores: analysisData.scores,
        search_gap_summary: analysisData.searchGapSummary,
        one_feature_recommendation: analysisData.oneFeatureRecommendation
      }])
      .select()
      .single();
    ...
    if (queryError) {
      ...
      await Promise.all([
        supabaseAdmin.from("generated_apps").insert(
          analysisData.appIdeas.map((app: any) => ({ 
            query_id: queryId, 
            ...app 
          }))
        ),
        supabaseAdmin.from("generated_specs").insert([{ 
          query_id: queryId, 
          content: analysisData.mvpSpec 
        }]),
        supabaseAdmin.from("seo_video_packs").insert([{ 
          query_id: queryId, 
          seo_data: analysisData.seoPack,
          video_ideas: analysisData.videoIdeas
        }]),
      ]);
    }
